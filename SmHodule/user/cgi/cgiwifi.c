/*
Cgi/template routines for the /setup url.
*/
...
int ICACHE_FLASH_ATTR cgiWiFiConnect(HttpdConnData *connData) {
	...
	httpdFindArg(connData->postBuff, "stpasswd", stpasswd, sizeof(stpasswd));

	os_strncpy((char*)stconf.ssid, essid, 32);
	os_strncpy((char*)stconf.password, stpasswd, 64);
	os_sprintf((uint8_t *) Setup.WifiStPassword, "%s", stpasswd);
...
#ifdef DEMO_MODE
	httpdRedirect(connData, "/setup/wifi.tpl");
#else
...
//This cgi resets the access point password
int ICACHE_FLASH_ATTR cgiWiFiApReset(HttpdConnData *connData) {
	char appasswd[128];
	...
	httpdFindArg(connData->postBuff, "appasswd", appasswd, sizeof(appasswd));

	if ( os_strlen(appasswd)  )
		os_sprintf((uint8_t *) Setup.WifiApPassword, "%s", appasswd);
...
void ICACHE_FLASH_ATTR tplWlan(HttpdConnData *connData, char *token, void **arg) {
	...
	if (os_strcmp(token, "WiFiMode")==0) {
		...
		if (x==2) {
			...
			os_strcpy(buff, "Click here for <a href=\"setmode.cgi?mode=1\">STA</a>, <a href=\"setmode.cgi?mode=3\">STA+AP</a> mode.");
		}
	}
