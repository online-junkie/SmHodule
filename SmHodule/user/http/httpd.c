...
static HttpdConnData ICACHE_FLASH_ATTR *httpdFindConnData(void *arg) {
	...
	/*
	// allow the device to close the sessions
	int connectionsAlive = 0;
	for (i=0; i<MAX_CONN; i++) {
		if (connData[i].conn != NULL) { connectionsAlive++; }
	}
	if ( !connectionsAlive ) {
	os_printf("Httpd re-init, conn=%p\n", &httpdConn);
#ifdef PLATFORM_DEBUG
	os_printf("max heap available at re-init: %u\r\n",system_get_free_heap_size());
#endif
...
void ICACHE_FLASH_ATTR httpdStartResponse(HttpdConnData *conn, int code) {
	char buff[128];
	int len;
	len=os_sprintf(buff, "HTTP/1.0 %d OK\r\nServer: SmHodule-httpd/"HTTPDVER"\r\n", code);
...
void ICACHE_FLASH_ATTR httpdRedirect(HttpdConnData *conn, char *newUrl) {
	char buff[1024];
	int len;
	len=os_sprintf(buff, "HTTP/1.1 302 Found\r\nLocation: %s\r\n\r\nMoved to %s\r\n", newUrl, newUrl);
	httpdSend(conn, buff, len);
}
...
int ICACHE_FLASH_ATTR httpdSend(HttpdConnData *conn, const char *data, int len) {
	...
	if (conn->priv->sendBuffLen+len>MAX_SENDBUFF_LEN) {
		os_printf("send data is exceeding buffer by %d",(MAX_SENDBUFF_LEN - (conn->priv->sendBuffLen+len)));
		return 0;
	}
...
static void ICACHE_FLASH_ATTR httpdSendResp(HttpdConnData *conn) {
	...
	while (builtInUrls[i].url!=NULL && conn->url!=NULL) {
		int match=0;
// we are very verbose....
//os_printf("%s == %s?:%p\n", builtInUrls[i].url, conn->url, conn);
	...
	httpdSend(conn, httpNotFoundHeader, os_strlen(httpNotFoundHeader));
	conn->cgi=NULL; //mark for destruction
}
...
// reworked recon cb method to check what happens to the connections
static void ICACHE_FLASH_ATTR httpdReconCb(void *arg, sint8 err) {
	HttpdConnData *conn=httpdFindConnData(arg);
    struct espconn *pespconn = conn->conn;
    os_printf("recon\r\n");
